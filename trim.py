...
class Trim(Constants):
    ...
    def alpha_control(self):
        ...
        alpha = self.longitudinal_cyclic - atan(self.w / self.u)
    ...
    def thrust_coefficient_elem(self, lambda_i):
        ...
        lambda_c = self.inflow_ratio_control
        theta0 = self.collective_pitch
        return (0.25 * cla * sigma) * ((2.0/3.0) * theta0 * (1 + (1.5 * mu**2)) - (lambda_c + lambda_i))
    ...
    def thrust_coefficient_glau(self, lambda_i):
        ...
        return lambda_i * sqrt(((v / (omega * r)) * cos(self.alpha_control - a1))**2 +
                               ((v / (omega * r)) * sin(self.alpha_control - a1) + lambda_i)**2)

    @Attribute
    def inflow_ratio(self):
        """ Utilizes a minimizer to compute the inflow ratio as discussed in the lecture slides """

        def func(lambda_i):
            return self.thrust_coefficient_elem(lambda_i) - self.thrust_coefficient_glau(lambda_i)

        return float((fsolve(func, x0=np.array([1])))[0])
    ...
    @Attribute
    def q_dot(self):
        # TODO Change the 2.0 to the correct height of the disk plane from the c.g.
        return (-self.thrust / self.inertia_blade) * 2.0 * sin(self.longitudinal_cyclic - self.longitudinal_disk_tilt)
...
if __name__ == '__main__':
    obj = Trim(20, 0.0, 0.0, radians(50.0))
